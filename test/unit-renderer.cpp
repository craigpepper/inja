...
TEST_CASE("types") {
    ...
    SUBCASE("loops") {
        ...
        CHECK(env.render("{% for type, name in relatives %}{{ loop.index1 }}: {{ type }}: {{ name }}{% if loop.is_last == false %}, {% endif %}{% endfor %}", data) == "1: mother: Maria, 2: brother: Chris, 3: sister: Jenny");
